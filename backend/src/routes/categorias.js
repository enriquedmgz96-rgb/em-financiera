...
router.get('/', async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(
      'SELECT * FROM categorias_tasa WHERE activo = TRUE AND periodicidad = $1 ORDER BY tasa_mensual',
      [periodicidad]
    );
    res.json(rows);
  } catch (err) { next(err); }
...
router.post('/', async (req, res, next) => {
  const { nombre, tasa_mensual, color, periodicidad } = req.body;
  if (!nombre || !tasa_mensual) {
    return res.status(400).json({ error: 'nombre y tasa_mensual son requeridos' });
  }
  try {
    const { rows } = await pool.query(
      'INSERT INTO categorias_tasa (nombre, tasa_mensual, color, periodicidad) VALUES ($1, $2, $3, $4) RETURNING *',
      [nombre.trim(), tasa_mensual, color || 'azul', periodicidad || 'mensual']
    );
